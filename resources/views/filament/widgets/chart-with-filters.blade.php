@php
    ...
    $dateFilters = method_exists($this, 'getDateFilters') ? $this->getDateFilters() : [];
    $genderFilters = method_exists($this, 'getGenderFilters') ? $this->getGenderFilters() : [];
@endphp

<x-filament-widgets::widget class="fi-wi-chart">
    <x-filament::section :description="$description" :heading="$heading">
        @if (!empty($dateFilters) || !empty($genderFilters))
            <x-slot name="headerEnd">
                <div class="flex flex-wrap items-center gap-2 w-max sm:-my-2">
                    @if (!empty($genderFilters))
                        <x-filament::input.wrapper
                            inline-prefix
                            wire:target="genderFilter"
                            class="w-max"
                        >
